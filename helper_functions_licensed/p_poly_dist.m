...
% Revision history:
% Sept. 12 2024 - fixed a bug that incorrectly sets the distances of
% out-of-polygon points to negative
...
%   if ((xv(1) ~= xv(nv)) && (yv(1) ~= yv(nv)))   
   if ((abs(xv(1) - xv(nv)) > 100*eps) && (abs(yv(1) - yv(nv)) > 100*eps))
      xv1 = [xv(:)' xv(1)];
      yv1 = [yv(:)' yv(1)];
      nv = nv + 1;
   end
...
idx = find(vds < 10*eps);
if(~isempty(idx))
   error('Points %i of the polyline are identical',idx);   
%   error(['Points ' num2str1(idx,0) ' of the polyline are identical']);
end
