function [vol_handle]=VoxelPlotter(VoxelMat,Vox_Size,fcol,fcol2)
    %detect the external voxels and faces
    vol_handle=0;
    if ~exist('Vox_Size','var') || isempty(Vox_Size)
        Vox_Size = 1;
    end
    if ~exist('fcol','var') || isempty(fcol)
        fcol = 'r';
    end
    if ~exist('fcol2','var') || isempty(fcol2)
        fcol2 = 'k';
    end
    ...
    vol_handle=patch(FV,'FaceColor',fcol,'EdgeColor',fcol2);
