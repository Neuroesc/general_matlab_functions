...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% FUNCTION BODY
    ...
    for ii = 1
        ...
        for i = 1:num_grid_cells
            ...
            % edges = 0:binsize:boxsize;
            % occupancy = histcounts2(trajectory.x, trajectory.y, edges, edges);
            % spike_counts = histcounts2(spikes_x, spikes_y, edges, edges);
    
            subplot(3, num_grid_cells, i);
            plot(pos(:,1),pos(:,2),'k'); hold on;
            plot(spk(:,1),spk(:,2),'r.','MarkerSize',20);         
            xlabel('X (cm)'); ylabel('Y (cm)');
    
            subplot(3, num_grid_cells, i + num_grid_cells);
            imagesc( rmap);
            axis image xy
            colormap jet;
            ...
            % Plot FFT Magnitude
            subplot(3, num_grid_cells, i + num_grid_cells.*2);
            imagesc(fft_mag);
            axis image xy; colormap jet;
            if ii==1
                title(sprintf('2D FFT Magnitude\nTrue Spac: %.1fcm', cells.Grid(i).params.spacing));
            else
                title(sprintf('2D FFT Magnitude'));
            end
            xlabel('Freq X'); ylabel('Freq Y');

            % % Plot FFT Magnitude
            % subplot(4, num_grid_cells, i + num_grid_cells.*3);
            % imagesc(idealised_rmap);
            % axis image xy
            % colormap jet;
            % xlabel('X (cm)'); ylabel('Y (cm)');
